...
@section('title', 'Add Quiz Category Questions')

@section('user',Auth::user()->name)

@section('breadcrumb')
<li class="breadcrumb-item"><a href="/quiz/category/question/list">Manage Quiz Creation</a></li>
<li class="breadcrumb-item active">Add Quiz Category Questions</li>
@endsection
...
<script>
  $(function() {
    ...
    $(".cat_ques,.cat_option1,.cat_option2,.cat_option3,.cat_option4,.ques_ans,.time_p_ques,.point_question").each(function (item) {
        $(this).rules("add", {
            required: true,
        });
    });
            ...
            '<div class="row div_tocopy ques_count appended-content">'+
                    ...
                    '<div class="col-md-3 ">'+
                        '<div class="form-group">'+
                            ...
                            '<input type="text" class="cat_option1 form-control input-css " id="cat_option1_'+count+'"  name="cat_option1[]" placeholder="Option 1">'+
                        '</div>'+               
                    '</div>'+
                    '<div class="col-md-3 ">'+
                        '<div class="form-group">'+
                            '<label for="cat_option2">{{__("Option 2")}}<sup>*</sup></label><br>'+
                            '<input type="text" class="cat_option2 form-control input-css " id="cat_option2_'+count+'" placeholder="Option 2" name="cat_option2[]">'+
                        '</div>'+ 
                    '</div>'+
                    '<div class="col-md-3 ">'+
                        '<div class="form-group">'+
                            '<label for="cat_option3">{{__("Option 3")}}<sup>*</sup></label><br>'+
                            '<input type="text" class="cat_option3 form-control input-css" id="cat_option3_'+count+'"  name="cat_option3[]" placeholder="Option 3">'+
                        '</div>'+              
                    '</div>'+
                    '<div class="col-md-3 ">'+
                        '<div class="form-group">'+
                            '<label for="cat_option4">{{__("Option 4")}}<sup>*</sup></label><br>'+
                            '<input type="text" class="cat_option4 form-control input-css @error("cat_option4") is-invalid @enderror" id="cat_option4_'+count+'"  name="cat_option4[]" placeholder="Option 4">'+
                        '</div> '+
                    '</div>'+
                    '<div class="col-md-4 ">'+
                    ...
                    '<div class="col-md-4 ">'+
                        ...
                        '<input type="number" class="point_question form-control input-css " name="point_question[]" id="point_question'+count+'" min="0" value="" autocomplete="off">'+              
                    '</div>'+
                    '<div class="col-md-4 ">'+
                        '<label for="cat_option4">{{__("Question Time")}} (Minute:Seconds)<sup>*</sup></label><br>'+
                        '<input type="text" class="time_p_ques datetimepicker-input form-control input-css " name="time_p_ques[]" id="time_p_ques_'+count+'" autocomplete="off" data-target="#time_p_ques'+count+'" data-toggle="datetimepicker" onblur="ques_time_change()">'+         
                    '</div>'+
                '</div>');
                $('.time_p_ques').datetimepicker({
                  showSecond: true,
           ...
           <div class="row">
                <div class="col-md-12">
                    <div class="col-md-11 card-head-left">
                      <h5 class="m-0">Add Quiz Category Questions</h5>
                    </div>
                </div>
            </div>
        </div>  
        <form  action="{{url('/quiz/category/questions/create')}}" enctype="multipart/form-data" method="POST" id="quiz_create_form" files="true">
               ...
               <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="select2bs4 form-control @error('cat_name') is-invalid @enderror" id="cat_name" name="cat_name" >
                            <option value="">Select Quiz Category</option>
                            @foreach($category as $cat)
                                <option value="{{$cat['quiz_category_id']}}" {{ (old('cat_name')== $cat['quiz_category_id'])?'selected':''}}>{{$cat['quiz_category_name']}}</option>
                            @endforeach
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="capital form-control input-css @error('quiz_title') is-invalid @enderror" name="quiz_title"
                            value="{{ old('quiz_title') }}" autocomplete="off">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="quiz_total_time datetimepicker-input form-control input-css @error('quiz_total_time') is-invalid @enderror" name="quiz_total_time" id="quiz_total_time" value="{{old('quiz_total_time')}}" autocomplete="off" data-target="#quiz_total_time" data-toggle="datetimepicker" onchange="perQuizTime()">
                        </div> 
                        <span class="invalid-feedback" role="alert" id="number-err" style="display: none;"></span>
                ...
                <div class="row div_tocopy ques_count">
                    <div class="col-md-12">
                        ...
                        <textarea class="cat_ques form-control input-css @error('cat_ques.0') is-invalid @enderror" name="cat_ques[]" autocomplete="off" placeholder="Enter Quiz Question here...">{{old('cat_ques.0')}}</textarea>
                            @error('cat_ques.0')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="col-md-4 ">
                        <label for="cat_option4">{{__('Question Time')}} (Minute:Seconds)<sup>*</sup></label><br>
                        <input type="text" class="time_p_ques datetimepicker-input form-control input-css @error('time_p_ques') is-invalid @enderror" name="time_p_ques[]" id="time_p_ques_0" value="{{old('time_p_ques.0')}}" autocomplete="off" data-target="#time_p_ques_0" data-toggle="datetimepicker" onblur="ques_time_change()">
                        @error('time_p_ques.0')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                            ...
                            @for($i=1;$i < count(old('cat_ques'));$i++)
                                <div class="row div_tocopy ques_count appended-content">
                                    <div class="col-md-12">
                                        ...
                                        <textarea class="cat_ques form-control input-css @error('cat_ques.*') is-invalid @enderror" name="cat_ques[]" autocomplete="off" placeholder="Enter Quiz Question here...">{{(old('cat_ques')[$i])}}</textarea>
                                            @error('cat_ques.*')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                    ...
                                    <div class="col-md-4 ">
                                     ...
                                     <div class="col-md-4 ">
                                        <label for="cat_option4">{{__('Question Coins')}}<sup>*</sup></label><br>
                                        <input type="number" class="point_question form-control input-css @error('point_question.*') is-invalid @enderror" name="point_question[]" id="point_question_{{$i}}" min="0" value="{{old('point_question'[$i])}}" autocomplete="off">
                                    ...
                                    <div class="col-md-4 ">
                                        <label for="cat_option4">{{__('Question Time')}} (Minute:Seconds)<sup>*</sup></label><br>
                                        <input type="text" class="time_p_ques datetimepicker-input form-control input-css @error('time_p_ques.*') is-invalid @enderror" name="time_p_ques[]" id="time_p_ques_{{$i}}" value="{{old('time_p_ques')[$i]}}" autocomplete="off" data-target="#time_p_ques_{{$i}}" data-toggle="datetimepicker" onblur="ques_time_change()">
                                        @error('time_p_ques.*')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
